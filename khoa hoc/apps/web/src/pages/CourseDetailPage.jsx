...
export default function CourseDetailPage({ courseId, currentUser, onNavigateToLearn, onUpdateUser, navigateTo, onCheckoutCourse, isDashboard = false }) {
  ...
  useEffect(() => {
    ...
    api.getCourseById(courseId)
      .then(res => {
        if (!active) return;
        if (res) {
          const mappedCourse = mapDatabaseCourseToMockFormat(res);
          ...
          if (isDashboard && currentUser) {
            // Auto redirect to learn page if owned or free (dashboard only)
            if (userEnrolled || mappedCourse.priceSale === 0) {
              const firstLessonId = mappedCourse?.curriculum?.[0]?.lessons?.[0]?.id;
              const firstPreviewLesson = mappedCourse?.curriculum
                ?.flatMap(s => s.lessons)
                ?.find(l => l.isPreview)?.id || firstLessonId;
              
              const targetLessonId = userEnrolled ? firstLessonId : firstPreviewLesson;

              if (mappedCourse.priceSale === 0 && !userEnrolled) {
                // Free course auto-enrollment first
                enrollmentService.enrollCourse(currentUser.id, mappedCourse.id, 0)
                  .then(() => {
                    if (onUpdateUser) {
                      const activeUnlocked = currentUser.unlockedCourses || [];
                      onUpdateUser({
                        ...currentUser,
                        unlockedCourses: [...activeUnlocked, Number(mappedCourse.id), mappedCourse.id.toString()]
                      });
                    }
                    onNavigateToLearn(mappedCourse.id, targetLessonId);
                  })
                  .catch(err => {
                    console.error('Failed to auto enroll free course:', err);
                    onNavigateToLearn(mappedCourse.id, targetLessonId);
                  });
              } else {
                // Already owned, redirect immediately
                onNavigateToLearn(mappedCourse.id, targetLessonId);
              }
              return; // stop execution and don't change loading state to prevent flash
            }
          }
          ...
          if (currentUser) {
            ...
            if (saved && active) setCompletedLessons(JSON.parse(saved));
          }

          // Use reviews directly from database/mapped course
  ...
  const handleEnroll = async (action) => {
    ...
    if (action === 'buy') {
      if (course.priceSale === 0) {
        ...
        try {
          await enrollmentService.enrollCourse(currentUser.id, course.id, 0);
          setIsOwned(true);
        ...
        // Paid checkout flow using SePay Modal
        if (onCheckoutCourse) {
